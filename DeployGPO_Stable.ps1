...
$DomainComputersSID = (Get-ADDomain -Current LocalComputer).DomainSID.Value + '-515'
$DomainComputersName = (Get-ADGroup -Filter "SID -eq `'$DomainComputersSID`'").Name
$DomainControllersSID = (Get-ADDomain -Current LocalComputer).DomainSID.Value + '-516'
...
try {
    ...
    New-GPO -Domain $DomainFQDN -Name $GPONamewithTimestamp -ErrorAction Stop
...
try {
    Import-GPO -Domain $DomainFQDN -Path $BackupPath -TargetName $GPONamewithTimestamp -BackupId $Backupid -ErrorAction Stop
